    ...
    for batch_id, (batch_labels, batch_sentences) in tqdm(enumerate(data_iter)):
        ...
        for i, (a, b, c) in enumerate(zip(batch_clean_sentences, batch_corrupt_sentences, batch_predictions)):
            results.append({"id": line_index + i, "original": a, "noised": b, "predicted": c, "topk": [],
                            "topk_prediction probs": [], "topk_reranker_losses": []})
...
def process_and_save(input_string, output_file="output.txt"):
    ...
    corrected_sentences, splitters = spell_checking_on_sents(model, vocab, device, normalizer, input_string)

    # Reconstruct the output string with original splitters (preserving \n)
    corrected_output = ""
    for i, (original, corrected) in enumerate(corrected_sentences):
        corrected_output += corrected
        if i < len(splitters):  # Add the splitter after each sentence
            corrected_output += splitters[i]
