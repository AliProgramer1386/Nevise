...
def spell_checking_on_sents(model, vocab, device, normalizer, txt):
    ...
    # Reconstruct the corrected text with original line breaks
    corrected_text = []
    for line, splitter in zip(greedy_results, splitters):
        corrected_text.append(line['predicted'])
        # Append the splitter (which includes \n if it was present in the input)
        corrected_text.append(splitter)
    
    # Join the corrected text while preserving the original line breaks
